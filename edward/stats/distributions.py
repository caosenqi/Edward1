...
from edward.util import dot, get_dims, lbeta
...
class Beta:
    ...
    def logpdf(self, x, a, b):
        ...
        return (a-1) * tf.log(x) + (b-1) * tf.log(1-x) - \
               lbeta(tf.pack([a, b]))
...
class Binom:
    ...
    def logpmf(self, x, n, p):
        ...
        p = tf.cast(tf.squeeze(p), dtype=tf.float32)
        return tf.lgamma(n + 1.0) - tf.lgamma(x + 1.0) - \
               tf.lgamma(n - x + 1.0) + \
...
class InvGamma:
    ...
    def logpdf(self, x, alpha, scale=1):
        ...
        scale = tf.cast(tf.squeeze(scale), dtype=tf.float32)
        return tf.mul(alpha, tf.log(scale)) - tf.lgamma(alpha) + \
